...
void Daftarlagu(Playlist *p, int indexLagu) {
    ...
    cout << "Judul\t: " << ptrLagu->judulLagu << endl;
    cout << "Artis\t: " << ptrLagu->artis << endl;
    cout << "Genre\t: " << ptrLagu->genre << endl;
    cout << "Tahun\t: " << ptrLagu->tahun << endl;
    cout << "-----------------------------------------" << endl;
...
bool interpolationSearchJumlahLagu(const Playlist* arr, int n, int target, int& idx){
    ...
    while(low <= high && target >= arr[low].jumlahlagu && target <= arr[high].jumlahlagu)
    {
        ...
        double valLow = arr[low].jumlahlagu;
        double valHigh = arr[high].jumlahlagu;
...
void menuSearching(User *u){
    ...
    {
        cout << "Info\t\t: Anda belum memiliki Playlist untuk dicari" << endl;
        return;
    }
    ...
    if(metode == 1)
    {
        ...
        for(int p = 0; p < u->jumlahPlaylist; p++){
            for(int l = 0; l < u->musiklist[p].jumlahlagu; l++){
                if(linearSearchArtis(&u->musiklist[p].lagu[l], 1, artisCari, idx))
                {
                    ...
                    cout << "Judul\t: " << u->musiklist[p].lagu[l].judulLagu << endl;
                    cout << "Artis\t: " << u->musiklist[p].lagu[l].artis << endl;
                    cout << "Genre\t: " << u->musiklist[p].lagu[l].genre << endl;
                    cout << "Tahun\t: " << u->musiklist[p].lagu[l].tahun << endl;
                    cout << "-----------------------------------------" << endl;
                    foundAny = true;
                }
            }
        ...
        if(!foundAny){
            cout << "Info\t\t: Data artis tidak ditemukan" << endl;
        }
        return;
    } 
    else if(metode == 2)
    {
        ...
        if(pIdx < 1 || pIdx > u->jumlahPlaylist){
            cout << "Info\t\t: Playlist tidak valid" << endl;
            return;
        }
        
        string judulCari;
        cout << "Masukkan judul lagu: "; cin.ignore(); getline(cin, judulCari);
        ...
        if(found){
            cout << endl << "Info\t\t: Data ditemukan" << endl;
            cout << "Judul\t: " << temp[idx].judulLagu << endl;
            cout << "Artis\t: " << temp[idx].artis << endl;
            cout << "Genre\t: " << temp[idx].genre << endl;
            cout << "Tahun\t: " << temp[idx].tahun << endl;
        } else {
            cout << "Info\t\t: Judul lagu tidak ditemukan" << endl;
        }
    }
    else if(metode == 3)
    {
        ...
        if(found)
        {
            ...
            cout << "Info\t\t: Tahun tidak ditemukan" << endl;
        }
    }
    else if(metode == 4)
    {
        int jumlahCari;
        cout << "Masukkan jumlah lagu playlist yang dicari: ";
        cin >> jumlahCari;
        vector<Playlist> temp(u->musiklist, u->musiklist + u->jumlahPlaylist);
        sort(temp.begin(), temp.end(), [](const Playlist& a, const Playlist& b){ 
            return a.jumlahlagu < b.jumlahlagu; 
        });
        
        found = interpolationSearchJumlahLagu(temp.data(), temp.size(), jumlahCari, idx);
        if(found)
        {
            cout << '\n' << "Info\t\t: Playlist ditemukan" << endl;
            cout << "Nama Playlist: " << temp[idx].judul << endl;
            cout << "Jumlah Lagu  : " << temp[idx].jumlahlagu << endl;
        }
        else{
            cout << "Info\t\t: Playlist dengan jumlah lagu tersebut tidak ditemukan" << endl;
        }
    }
    else{
        cout << "Info\t\t: Pilihan metode tidak valid" << endl;
    }
}
...
void menuSorting(User *u){
    if(u->jumlahPlaylist == 0)
    {
        cout << "Info\t\t: Anda belum memiliki Playlist untuk diurutkan" << endl;
        return;
    }
    ...
    if(opsi == 1)
    {
        ...
        cout << "Info\t\t: Playlist berhasil diurutkan secara ascending" << endl;
        for(int i=0; i<u->jumlahPlaylist; i++){
            cout << i+1 << ". " << u->musiklist[i].judul << endl;
        }
    }
    else if(opsi == 2)
    {
        ...
        cout << "Pilih nomor playlist: ";
        cin >> idx;
        if(idx >= 1 && idx <= u->jumlahPlaylist && u->musiklist[idx-1].jumlahlagu > 0)
        {
            selectionSortLaguDSC(&u->musiklist[idx-1]);
            cout << "Info\t\t: Lagu berhasil diurutkan secara descending berdasarkan tahun" << '\n';
            Daftarlagu(&u->musiklist[idx-1], 0);
        }
        else{
            cout << "Info\t\t: Playlist tidak ditemukan atau tidak memiliki lagu" << endl;
        }
    }
    else if(opsi == 3){
        int idx;
        ...
        if(idx >= 1 && idx <= u->jumlahPlaylist && u->musiklist[idx-1].jumlahlagu > 0)
        {
            ...
            cout << "Info\t\t: Lagu berhasil diurutkan secara ascending berdasarkan judul lagu" << endl;
            Daftarlagu(&u->musiklist[idx-1], 0);
        }
        else{
            cout << "Info\t\t: Playlist tidak ditemukan atau tidak memiliki lagu" << endl;
